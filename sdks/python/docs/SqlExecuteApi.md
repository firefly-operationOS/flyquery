...
# **execute_stream**
> str execute_stream(x_tenant_id, x_workspace_id, sql_execute_request, x_correlation_id=x_correlation_id, idempotency_key=idempotency_key)
...
async with flyquery_sdk.ApiClient(configuration) as api_client:
    ...
    try:
        # Execute SQL and stream progress as Server-Sent Events.
        api_response = await api_instance.execute_stream(x_tenant_id, x_workspace_id, sql_execute_request, x_correlation_id=x_correlation_id, idempotency_key=idempotency_key)
        print("The response of SqlExecuteApi->execute_stream:\n")
        pprint(api_response)
    except Exception as e:
