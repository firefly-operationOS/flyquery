...
# **batch**
> BatchQueryResponse batch(x_tenant_id, x_workspace_id, batch_query_request, x_correlation_id=x_correlation_id, idempotency_key=idempotency_key)
...
async with flyquery_sdk.ApiClient(configuration) as api_client:
    ...
    try:
        # Run multiple NL questions in parallel through the full pipeline.
        api_response = await api_instance.batch(x_tenant_id, x_workspace_id, batch_query_request, x_correlation_id=x_correlation_id, idempotency_key=idempotency_key)
...
# **stream**
> str stream(x_tenant_id, x_workspace_id, query_request, x_correlation_id=x_correlation_id, idempotency_key=idempotency_key)
...
async with flyquery_sdk.ApiClient(configuration) as api_client:
    ...
    try:
        # Run the full pipeline as a Server-Sent Events stream.
        api_response = await api_instance.stream(x_tenant_id, x_workspace_id, query_request, x_correlation_id=x_correlation_id, idempotency_key=idempotency_key)
        print("The response of QueryApi->stream:\n")
        pprint(api_response)
    except Exception as e:
