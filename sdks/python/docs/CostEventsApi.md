# flyquery_sdk.CostEventsApi

All URIs are relative to *http://localhost*

Method | HTTP request | Description
------------- | ------------- | -------------
[**list_events**](CostEventsApi.md#list_events) | **GET** /api/v1/cost-events | List cost events for the caller&#39;s workspace, newest first.


# **list_events**
> PaginatedCostEventRead list_events(x_tenant_id, x_workspace_id, actor=actor, model=model, operation=operation, date_from=date_from, date_to=date_to, limit=limit, offset=offset, x_correlation_id=x_correlation_id)
...
async with flyquery_sdk.ApiClient(configuration) as api_client:
    ...
    api_instance = flyquery_sdk.CostEventsApi(api_client)
    ...
    try:
        # List cost events for the caller's workspace, newest first.
        api_response = await api_instance.list_events(x_tenant_id, x_workspace_id, actor=actor, model=model, operation=operation, date_from=date_from, date_to=date_to, limit=limit, offset=offset, x_correlation_id=x_correlation_id)
        print("The response of CostEventsApi->list_events:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling CostEventsApi->list_events: %s\n" % e)
```
