...
# **execute**
> SqlExecuteResponse execute(x_agent_token, sql_execute_request, x_correlation_id=x_correlation_id, idempotency_key=idempotency_key)
...
async with flyquery_sdk.ApiClient(configuration) as api_client:
    ...
    try:
        # Execute SQL directly (agent-tier).
        api_response = await api_instance.execute(x_agent_token, sql_execute_request, x_correlation_id=x_correlation_id, idempotency_key=idempotency_key)
...
# **execute_stream**
> str execute_stream(x_agent_token, sql_execute_request, x_correlation_id=x_correlation_id, idempotency_key=idempotency_key)
...
async with flyquery_sdk.ApiClient(configuration) as api_client:
    ...
    try:
        # Execute SQL as SSE stream (agent-tier).
        api_response = await api_instance.execute_stream(x_agent_token, sql_execute_request, x_correlation_id=x_correlation_id, idempotency_key=idempotency_key)
        print("The response of AgentSqlExecuteApi->execute_stream:\n")
        pprint(api_response)
    except Exception as e:
