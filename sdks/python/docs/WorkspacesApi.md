...
# **list_workspaces**
> PaginatedWorkspaceRead list_workspaces(x_tenant_id, x_workspace_id, q=q, slug=slug, status=status, limit=limit, offset=offset, x_correlation_id=x_correlation_id)
...
async with flyquery_sdk.ApiClient(configuration) as api_client:
    ...
    try:
        # Search/filter workspaces for the caller's tenant.
        api_response = await api_instance.list_workspaces(x_tenant_id, x_workspace_id, q=q, slug=slug, status=status, limit=limit, offset=offset, x_correlation_id=x_correlation_id)
