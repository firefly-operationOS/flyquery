# coding: utf-8

"""
    flyquery

    Operational Structured-Data Intelligence (upload-driven) -- multi-tenant ingestion + Text-to-SQL over user-uploaded structured files (CSV / TSV / XLSX / XLS / ODS / JSON / JSONL / Parquet / Avro / ORC / Arrow / Feather + .gz/.zip/.bz2 variants). Materialises uploads to Parquet on object storage; indexes a long-lived schema knowledge base; answers natural-language questions via a multi-agent pipeline. Part of Firefly OperationOS.

    The version of the OpenAPI document: 26.5.4
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from flyquery_sdk.api.queries_api import QueriesApi


class TestQueriesApi(unittest.IsolatedAsyncioTestCase):
    """QueriesApi unit test stubs"""

    async def asyncSetUp(self) -> None:
        self.api = QueriesApi()

    async def asyncTearDown(self) -> None:
        await self.api.api_client.close()

    async def test_get_query(self) -> None:
        """Test case for get_query

        Fetch a single query with every candidate, retry, and model id.
        """
        pass

    async def test_get_query_result(self) -> None:
        """Test case for get_query_result

        Re-download a previously-executed query's preview + parquet.
        """
        pass

    async def test_list_queries(self) -> None:
        """Test case for list_queries

        List queries for the caller's workspace, newest first.
        """
        pass


if __name__ == '__main__':
    unittest.main()
