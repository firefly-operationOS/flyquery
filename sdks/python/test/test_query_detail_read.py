# coding: utf-8

"""
    flyquery

    Operational Structured-Data Intelligence (upload-driven) -- multi-tenant ingestion + Text-to-SQL over user-uploaded structured files (CSV / TSV / XLSX / XLS / ODS / JSON / JSONL / Parquet / Avro / ORC / Arrow / Feather + .gz/.zip/.bz2 variants). Materialises uploads to Parquet on object storage; indexes a long-lived schema knowledge base; answers natural-language questions via a multi-agent pipeline. Part of Firefly OperationOS.

    The version of the OpenAPI document: 26.5.4
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from flyquery_sdk.models.query_detail_read import QueryDetailRead

class TestQueryDetailRead(unittest.TestCase):
    """QueryDetailRead unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> QueryDetailRead:
        """Test QueryDetailRead
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `QueryDetailRead`
        """
        model = QueryDetailRead()
        if include_optional:
            return QueryDetailRead(
                ast_classification = '',
                candidates_json = [
                    null
                    ],
                chosen_candidate_index = 56,
                clarification_emitted = True,
                clarification_json = { },
                cost_cents = None,
                created_at = None,
                dataset_id = '',
                elapsed_ms = 56,
                error_json = { },
                executed_sql = '',
                execution_engine = 'duckdb',
                execution_status = '',
                finalised_at = None,
                id = '',
                model_critic = '',
                model_explainer = '',
                model_generation = '',
                model_grounding = '',
                pii_findings_json = { },
                prior_turn_ids = [
                    ''
                    ],
                question = '',
                retries = 56,
                row_count = 56,
                semantic_path_taken = '',
                table_id_snapshot_pins_json = { },
                tenant_id = '',
                workspace_id = ''
            )
        else:
            return QueryDetailRead(
                created_at = None,
                id = '',
                question = '',
                tenant_id = '',
                workspace_id = '',
        )
        """

    def testQueryDetailRead(self):
        """Test QueryDetailRead"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
