...
def build_object_store(settings: FlyquerySettings) -> ObjectStore:
    ...
    if kind == "gcs":
        if not _HAS_GCS:
            raise RuntimeError(
                "object_store=gcs but [gcs] extra not installed; run: uv sync --extra gcs"
            )
        from flyquery.core.services.storage.adapters.gcs import GcsObjectStore
        return GcsObjectStore(base=settings.object_store_base, presign_ttl_s=settings.object_store_presign_ttl_s)
    if kind == "azure":
        if not _HAS_AZURE:
            raise RuntimeError(
        ...
        return AzureBlobObjectStore(base=settings.object_store_base, presign_ttl_s=settings.object_store_presign_ttl_s)
